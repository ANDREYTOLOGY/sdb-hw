...
Master-Slave:
Однонаправленная репликация. Запись только на мастер, чтение с обоих серверов.   
Простая настройка, нет конфликтов, но мастер - единая точка отказа.

Master-Master:
Двунаправленная репликация. Запись на любой сервер, каждый является мастером и слейвом.  
Полная отказоустойчивость: если один из мастеров выйдет из строя, система продолжит работу, используя оставшиеся серверы.
